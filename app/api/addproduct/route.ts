...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!userData?.id) {
            return NextResponse.json({ error: "Kullanıcı kimliği alınamadı." }, { status: 400 });
        }
        ...
        if (files.length === 0 || !priceString) {
            return NextResponse.json({ error: "Fiyat ve en az bir resim gereklidir." }, { status: 400 });
        }
        
        // Fiyatı Sayıya Çevirme ve Kontrol (BURADA HATA OLABİLİRDİ!)
        const price = parseFloat(priceString);
        if (isNaN(price) || price <= 0) {
            return NextResponse.json({ error: "Geçerli bir fiyat girilmelidir." }, { status: 400 });
        }
        ...
        for (const file of files) {
            ...
            // Dosya adında güvenlik için sadece alfanümerik karakterlere izin verilebilir, 
            // ancak şimdilik mevcut mantığı koruyarak boşlukları alt çizgiye çeviriyoruz.
        ...
        console.error("API isteği işlenirken beklenmedik bir hata oluştu:", error);
        return NextResponse.json(
